...
void qt_range_query(QuadNode *node, double cx, double cy, double radius, int available_only, RangeResult *res) {
    ...
    if (node->is_leaf) {
        if (node->driver) {
            ...
            if ((dx*dx + dy*dy) <= (radius*radius)) {
                res->list[res->count++] = node->driver;
            }
        }
        return;
    }
    ...
    for (int i = 0; i < 4; i++){
        if (node->child[i]){
            qt_range_query(node->child[i], cx, cy, radius, available_only, res);
        }
    }
}

void qt_nearest_neighbor(QuadNode *node, double cx, double cy, int available_only, NNResult *best) {
    if (!node) return;
    ...
    if (node->child[first]){
        qt_nearest_neighbor(node->child[first], cx, cy, available_only, best);
    }
